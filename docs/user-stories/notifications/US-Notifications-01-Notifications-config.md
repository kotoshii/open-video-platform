...
* The aggregation mechanism — one open row per key, enforced by a partial unique index, closed when read — is written up
  step by step in [notification-aggregation.md](../../notification-aggregation.md).
...
* Creating, increasing and decreasing notifications must all be idempotent, because Kafka redelivers batches. Use the
  inbox approach from [kafka-dedup-and-inbox-pattern.md](../../kafka-dedup-and-inbox-pattern.md), and do not build this
...
* [notification-aggregation.md](../../notification-aggregation.md)
