...
`infra.yaml`, `postgres.shared.yaml`, `postgres.isolated.yaml`, `apps.yaml`, `observability.yaml` — and three top-level
files that assemble them with `include:`.

* `compose.yaml` (preview): infra, the shared Postgres, the apps.
* `compose.prod.yaml`: infra, one Postgres per service, the apps.
* `compose.dev.yaml`: infra and the shared Postgres, no apps.
