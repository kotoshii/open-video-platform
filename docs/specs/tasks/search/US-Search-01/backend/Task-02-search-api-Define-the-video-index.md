...
* Title and description are multi-fields: the base field with the `standard` analyzer, `.en` with `english` and `.uk`
  with `ukrainian`.
* Tags, the channel id, the channel's name and avatar, the duration in seconds, the upload date, and the
  age-restricted flag.
...
the UI's buckets are translated on the API side, not stored.
