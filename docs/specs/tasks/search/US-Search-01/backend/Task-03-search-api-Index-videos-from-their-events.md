...
and remove it when it is deleted or its channel is purged. A newly indexed video takes its channel's name and avatar
from `channel-api` over gRPC; refresh them from the channel-updated event, and the view count from the count events
the view count worker publishes.
